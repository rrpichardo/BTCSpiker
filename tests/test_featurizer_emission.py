"""Unit tests for ProductStream's split emission: immediate feature rows +
delayed outcome events.

Exercises ProductStream directly (pure, no Kafka). Run from the repo root with:
...
from featurizer import ProductStream  # noqa: E402
from btcspiker_ml.features import compute_future_vol  # noqa: E402

# ProductStream delegates all feature computation to the shared FeatureEngine
# (core_v1), so the row shape is core_v1's contract: the original tick-level
# columns plus feature_set_id/feature_schema_version, which the engine stamps
# on every row for train/serve parity.
...
def _new_stream(boot_id: str = "boot1") -> ProductStream:
    return ProductStream(FEATURE_SET_ID, HORIZON_SEC, VOL_THRESHOLD, boot_id)
...
def test_feature_row_emitted_same_call_no_delay():
    stream = _new_stream()
    feature_row, drained = stream.ingest(_tick("BTC-USD", 0.0, 100.0))
    ...
    feature_row_2, _ = stream.ingest(_tick("BTC-USD", 0.5, 100.5))
...
def test_outcome_arrives_only_after_horizon_and_matches_feature_id():
    ...
    first_row, drained = stream.ingest(_tick("BTC-USD", offsets[0], 100.0 + offsets[0]))
    ...
    # window using the pure helper on the same price sequence (mirrors the
    # price buffer the implementation would have built).
    ...
    last_t = offsets[-1]
    _, drained = stream.ingest(_tick("BTC-USD", last_t, 100.0 + last_t))
...
def test_timestamp_regression_bumps_epoch_and_drops_stale_pending():
    stream = _new_stream()
    stream.ingest(_tick("BTC-USD", 0.0, 100.0))
    stream.ingest(_tick("BTC-USD", 1.0, 100.1))

    assert stream.epoch == 0
    assert stream.seq == 2
    assert len(stream.engine.pending) == 2

    # Timestamp moves backwards -> ProductStream bumps its own epoch/seq;
    # FeatureEngine independently resets its rolling buffers on the same
    # regression signal.
    feature_row, drained = stream.ingest(_tick("BTC-USD", 0.5, 99.0))

    assert drained == []
    assert stream.epoch == 1
    assert feature_row["stream_epoch"] == 1
    assert feature_row["feature_id"] == "BTC-USD:boot1:1:0"
    assert stream.seq == 1  # incremented past the row we just emitted
    assert len(stream.engine.price_buffer) == 1
    assert len(stream.engine.spread_buffer) == 1
    assert len(stream.engine.timestamp_buffer) == 1
    # Only the post-regression row is pending; the epoch-0 rows are gone.
    assert len(stream.engine.pending) == 1
    ...
    while t < 0.5 + HORIZON_SEC + 1:
        _, drained = stream.ingest(_tick("BTC-USD", t, 99.0))
...
def test_vol_spike_label_is_strictly_greater_than_threshold():
    ...
    # Sanity: the stream's own vol_threshold (below the observed future_vol
...
def test_restarted_process_gets_distinct_feature_ids_from_prior_run():
    ...
    # mid-stream after a restart, so ProductStream is recreated fresh
    ...
    run1 = _new_stream(boot_id="run1")
    run1_row, _ = run1.ingest(_tick("BTC-USD", 0.0, 100.0))

    run2 = _new_stream(boot_id="run2")  # process restarted, group resumes mid-stream
