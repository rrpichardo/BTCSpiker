...
def test_nonpositive_trade_is_a_named_failure_instead_of_crash():
    start = datetime(2026, 1, 1, tzinfo=UTC)
    bad_trade = {"trade_id": "1", "event_time": start, "price": Decimal("0"), "size": Decimal("1")}
    report = audit_dataset(manifest([completion(start.date())]), book_intervals=intervals(1), trades=[bad_trade])
...
def test_causal_join_rejects_current_second_and_missing_book_evidence():
    ...
    report = audit_dataset(
        manifest([completion(timestamp.date())]), book_intervals=intervals(1),
        joined_ticks=[
            {"timestamp": timestamp, "book_observed_through": timestamp.replace(microsecond=0)},
...
def test_causal_join_rejects_segment_mismatch_and_crossed_gap():
    start = datetime(2026, 1, 1, tzinfo=UTC)
    report = audit_dataset(
        manifest([completion(start.date())]), book_intervals=intervals(1),
        replay_incidents=[(start + timedelta(seconds=1), start + timedelta(seconds=2))],
        joined_ticks=[{"timestamp": start + timedelta(seconds=3), "book_observed_through": start,
                       "segment_id": 2, "book_segment_id": 1}],
    )
...
def test_remote_manifest_partition_without_verified_receipt_fails_closed():
    data = RawDatasetManifest(**{**manifest([]).__dict__, "partitions": [
        {"remote_path": "raw/kind=trades/part.parquet", "sha256": "0" * 64, "row_count": 1}
    ]})
...
def test_exact_commit_verified_remote_receipt_is_accepted():
    ...
    data = RawDatasetManifest(**{**manifest([]).__dict__, "partitions": [{
        "remote_path": remote_path, "sha256": digest,
        "verified_receipt": {"remote_path": remote_path, "sha256": digest,
                             "revision": "b" * 40, "success": True},
    }]})
    report = audit_dataset(data)
...
def test_overlap_union_and_all_cut_types_are_counted_once():
    ...
    report = audit_dataset(
        ...
        book_intervals=[(start, start + timedelta(seconds=100)),
                        (start + timedelta(seconds=50), start + timedelta(seconds=150))],
        replay_incidents=[(start + timedelta(seconds=10), start + timedelta(seconds=20))],
        invalid_intervals=[(start + timedelta(seconds=30), start + timedelta(seconds=40))],
        excluded_intervals=[(start + timedelta(seconds=35), start + timedelta(seconds=55))],
    )
...
def test_report_files_are_machine_and_human_readable(tmp_path):
    ...
    state = BookState("BTC-USD", start, 1, 2, Decimal("1"), Decimal("1"), Decimal("2"), Decimal("1"))
    report = audit_dataset(manifest([]), book_intervals=intervals(1), trades=[trade], book_states=[state],
                           replay_incidents=[(start + timedelta(seconds=2), start + timedelta(seconds=3))],
                           excluded_intervals=[(start + timedelta(seconds=4), start + timedelta(seconds=5))],
                           output_dir=tmp_path)
    assert report.status == "FAIL"
    assert json.loads((tmp_path / "quality.json").read_text())["status"] == "FAIL"
    markdown = (tmp_path / "quality.md").read_text()
    for field in ("valid_seconds", "trade_count", "first_event", "last_event", "duplicate_count",
                  "sequence_range", "gap_incidents", "exclusions"):
...
def test_malformed_rows_still_emit_an_inspectable_fail_report(tmp_path):
    ...
    report = audit_dataset(
        manifest(completions()), book_intervals=intervals(),
        trades=[{"event_time": naive}], book_states=[{"observed_through": None}],
        label_windows=[{"start": naive}], output_dir=tmp_path,
    )
    assert report.status == "FAIL"
