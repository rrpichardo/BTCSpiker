...
def test_replay_does_not_carry_state_after_gap_without_post_gap_anchor(
    ...
    assert not [state for state in states if state.observed_through >= explicit_source_gap[1]]
...
def test_replay_publishes_raw_deltas_and_derived_states(tmp_path, replay_anchor, replay_delta_rows):
    ...
    assert json.loads(delta_table["changes_json"][0].as_py()) == [["bid", "90000.00", "1.35"]]
