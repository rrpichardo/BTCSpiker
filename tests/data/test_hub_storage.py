...
class FakeApi:
    ...
    def get_paths_info(self, **kwargs):
        return [
            type("PathInfo", (), {"lfs": type("Lfs", (), {"sha256": self.digest})()})()
        ]
...
def _partition(tmp_path):
    ...
    return PartitionRecord(
        path, sha256(b"partition").hexdigest(), 1, path.stat().st_size
    )
...
def test_upload_verifies_exact_commit_digest(tmp_path):
    ...
    receipt = PrivateHubStore.connect(api_factory=lambda: api).upload_partition(
        part, _remote_path(part)
    )
...
def test_upload_fails_when_committed_digest_mismatches_without_deleting_local_file(
    tmp_path,
):
    part = _partition(tmp_path)
    with pytest.raises(ValueError, match="checksum"):
        PrivateHubStore.connect(
            api_factory=lambda: FakeApi(digest="0" * 64)
        ).upload_partition(part, _remote_path(part))
...
def test_upload_fails_if_committed_revision_cannot_be_read(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="cannot read"):
        PrivateHubStore.connect(
            api_factory=lambda: FakeApi(digest=part.sha256, readable=False)
        ).upload_partition(part, _remote_path(part))
...
def test_existing_partition_without_exact_commit_sha_fails_closed(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="commit SHA"):
        PrivateHubStore.connect(api_factory=lambda: api).upload_partition(
            part, _remote_path(part)
        )
    assert api.uploads == 0
...
@pytest.mark.parametrize(
    ...
    [
        ...
        type(
            "UrlOnlyCommit",
            (),
            {"oid": None, "commit_url": f"https://huggingface.test/commit/{'e' * 40}"},
        )(),
    ],
)
