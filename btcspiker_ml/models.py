...
def build_model(family: str, params: dict[str, Any], seed: int, n_jobs: int):
    ...
    if family == "neural":
        _optional_library("torch", family)
        from btcspiker_ml.neural import SequenceWindowClassifier

        width = int(params.pop("hidden_width", 64))
        window = int(params.pop("window", 20))
        return Pipeline(
            [
                ("scale", StandardScaler()),
                ("model", SequenceWindowClassifier(**_with_defaults(params, {"hidden_size": width, "window": window, "seed": seed, "n_jobs": n_jobs}))),
            ]
        )
    if family == "lightgbm":
